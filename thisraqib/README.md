...
- **Vehicle / pedestrian detection** with runtime-selectable models (**YOLOv8s-VisDrone** (default) for CCTV/aerial views, or Faster R-CNN).
...
|---|---|---|---|
| Vehicle (default) | **YOLOv8s-VisDrone** | VisDrone 2018 (drone footage) | **Optimised for CCTV / oblique cameras** — finds significantly more vehicles than standard COCO models on TfL JamCams |
| Vehicle | **Faster R-CNN ResNet50-FPN-v2** | COCO 2017 | Two-stage, high recall on COCO classes, useful for dashcam comparison |
...
- Auto-switches the vehicle dropdown to **VisDrone** (the right pairing for pole-mounted CCTV).
...
└── server/
    ...
    │   ├── tailgating.py              2-second rule traffic analyser
    │   ├── vehicle.py                 YOLO11
...
| `GET` | `/models` | Lists vehicle / lane choices |
...
```json
{
  "image": "<base64 JPEG>",
  "frame_id": 42,
  "vehicle_model":      "visdrone",   // optional · default visdrone
