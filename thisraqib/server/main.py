...
Architecture:
  ...
  • Server runs two pre-trained deep learning models:
      1. YOLOv8s-VisDrone – vehicle / pedestrian detection (optimised for street mount)
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Pre-loading default detection models (VisDrone + YOLOPv2)…")
...
app = FastAPI(
    ...
    description=(
        "Real-time traffic safety detection using YOLOv8s-VisDrone (vehicle detection) "
...
def _run_pipeline(
    ...
    tailgating = vehicle.analyse_tailgating(vehicles, h, w)
...
class DetectRequest(BaseModel):
    ...
    vehicle_model: str | None = None          # "visdrone" | "fasterrcnn"
...
async def detect(req: DetectRequest):
    ...
    ``lane_model``; defaults are VisDrone + YOLOPv2.
    """
...
async def tfl_proxy(short_id: str, request: Request):
    """
    ...
    # Forward Range so partial requests reach S3 directly. Browsers send
    # ``Range: bytes=0-`` for any <video> source; without 206 + matching
    # Content-Range pass-through, Chrome over HTTPS/2 refuses to start
    # playback (the symptom: TfL feed selected, main view stays blank).
    ...
    try:
        upstream_resp = await client.send(
            client.build_request("GET", upstream, headers=fwd_headers),
            stream=True,
        )
    except Exception as exc:
        await client.aclose()
    ...
    if upstream_resp.status_code not in (200, 206):
        ...
        raise HTTPException(
            ...
            detail="TfL upstream returned unexpected status",
        )
    ...
    # 25-second cache: shorter than TfL's ~5-min refresh so a Pusher-driven
    # cache-bust ?_=… still pulls fresh content, long enough that the looping
    # <video> doesn't re-fetch on every restart of the 10-second clip.
