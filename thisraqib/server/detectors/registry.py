...
Model registry
==============
Lazy, cached factory for the vehicle detectors (YOLOv8s-VisDrone and
Faster R-CNN) and the lane detector (YOLOPv2). A detector is loaded
on its first request and kept resident for the rest of the process
lifetime — the registry trades a one-off several-second load per
model for constant-time retrieval thereafter.
...
Earlier RAQIB drafts also wired in YOLO11, RT-DETRv2 (transformer-
based vehicle detector) and UFLD (row-anchor lane detector). They
were removed late in the project: YOLO11 was a COCO-trained baseline
that underperformed on aerial CCTV, while RT-DETRv2 and UFLD were
redundant with the surviving Faster R-CNN / VisDrone (vehicles) and
YOLOPv2 (lanes), and exposing additional choices in the UI made the
...
VEHICLE_MODELS   = ("visdrone", "fasterrcnn")
LANE_MODELS      = ("yolopv2",)
DEFAULT_VEHICLE  = "visdrone"
...
_VEHICLE_ALIASES = {
    ...
    "yolo":            "visdrone",  # yolo now maps to visdrone
    "drone":           "visdrone",
    "traffic":         "visdrone",
    "trafficcam":      "visdrone",
