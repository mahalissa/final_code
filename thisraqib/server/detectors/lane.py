...
def _download_weights() -> None:
    """Fetch the official YOLOPv2 TorchScript checkpoint if missing with retries."""
    import os
    import time
    MODEL_DIR.mkdir(parents=True, exist_ok=True)
    if MODEL_PATH.exists():
        # Check if file is non-empty; if it's 0 bytes, it likely failed previously
        if MODEL_PATH.stat().st_size > 1000:
            logger.info(f"[LaneDetector] Weights already present at {MODEL_PATH}")
            return
        else:
            logger.warning(f"[LaneDetector] Found empty weights file at {MODEL_PATH}, re-downloading...")
            MODEL_PATH.unlink()

    logger.info(f"[LaneDetector] Downloading YOLOPv2 weights from {_WEIGHTS_URL}")
    print(f"Pre-fetching YOLOPv2 weights from {_WEIGHTS_URL}...")
    ...
    for attempt in range(1, 6):
        try:
            if curl:
                # Use -C - to resume if possible, though for a single file it might just restart.
                # Adding --fail to ensure curl exits with non-zero on 404/500 etc.
                result = subprocess.run(
                    [curl, "-L", "--fail", "-o", str(MODEL_PATH), _WEIGHTS_URL],
            ...
            # Verify file size (should be ~38MB)
            if MODEL_PATH.stat().st_size < 1000000: # Less than 1MB is likely a failure
                raise RuntimeError(f"Downloaded file too small: {MODEL_PATH.stat().st_size} bytes")
                
            print(f"YOLOPv2 weights ready (attempt {attempt}).")
            return
    ...
    raise RuntimeError(f"Failed to download YOLOPv2 weights after 5 attempts. Last error: {last_error}")
